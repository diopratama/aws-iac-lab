...
```bash
curl -u elastic:SuperSecureLabPass123! http://localhost:9200
```
...
1. **Index a Sample Document:**
   ```bash
   curl -u elastic:SuperSecureLabPass123! -X POST "http://localhost:9200/app-logs/_doc/1" \
...
2. **Retrieve the Document:**
   ```bash
   curl -u elastic:SuperSecureLabPass123! -X GET "http://localhost:9200/app-logs/_doc/1"
   ```

3. **Check Cluster Health Endpoint:**
   ```bash
   curl -u elastic:SuperSecureLabPass123! -X GET "http://localhost:9200/_cluster/health?pretty"
   ```
