...
* **Zero-Trust Network Perimeter & Zero Ingress Architecture:** Secure operator access for both shell terminal sessions and Elasticsearch port 9200 REST API requests that is tunneled securely via AWS Systems Manager (SSM) Session Manager (IAM-authenticated and fully audited). No open ingress ports are exposed to the public internet.
...
### A. Establish SSM Port Forwarding Session
Start a local port forwarding tunnel to port 9200 on the private instance:
...
Run the cURL test against your local tunneled port:

```bash
curl -k -u elastic:SuperSecureLabPass123! https://localhost:9200
```
...
### C. Validate CRUD Operations
1. **Index a Sample Document:**
   ```bash
   curl -k -u elastic:SuperSecureLabPass123! -X POST "https://localhost:9200/app-logs/_doc/1" \
     -H "Content-Type: application/json" \
     -d '{"service": "payment-gateway", "status": "completed", "latency_ms": 78}'
   ```

2. **Retrieve the Document:**
   ```bash
   curl -k -u elastic:SuperSecureLabPass123! -X GET "https://localhost:9200/app-logs/_doc/1"
   ```

3. **Check Cluster Health Endpoint:**
   ```bash
   curl -k -u elastic:SuperSecureLabPass123! -X GET "https://localhost:9200/_cluster/health?pretty"
   ```
